...
#                               Key certificate body: SW version - 32 bit
#                                                         SHA256 digest of Next certificate public key
...
class X509KeyCertificateGenerator:
    def __init__(self, key_cfg, certificate_version):
        self.key_cert_config = key_cfg
        self.logger = logging.getLogger()
        self.cert_version = certificate_version

    def create_certificate(self):
        self.logger.info("**** Creating X509 Key certificate ****")

        certificate = x509certificates.KeyX509Certificate(self.cert_version,
                                                          self.key_cert_config.hbk_id,
                                                          self.key_cert_config.cert_keypair,
                                                          self.key_cert_config.cert_keypair_pwd,
                                                          self.key_cert_config.next_cert_pubkey,
                                                          self.key_cert_config.nvcounter_val)

        self.logger.info("Write the certificate to cert-pkg output file as a DER encoded binary")
        with open(self.key_cert_config.cert_pkg, "wb") as bin_output_file:
...
if __name__ == "__main__":
    ...
    # create certificate
    cert_creator = X509KeyCertificateGenerator(key_certificate_config, cert_version)
