...
class X509ContentCertificateGenerator:
    ...
    def create_certificate(self):
        self.logger.info("**** Creating X509 Content certificate ****")
        certificate = x509certificates.ContentX509Certificate(self.cert_version, self.content_cert_config)

        self.logger.info("Write the certificate to cert-pkg output file as a DER encoded binary + additional data")
        with open(self.content_cert_config.cert_pkg, "wb") as bin_output_file:
...
if __name__ == "__main__":
    ...
    # create certificate
    cert_creator = X509ContentCertificateGenerator(content_certificate_config, cert_version)
