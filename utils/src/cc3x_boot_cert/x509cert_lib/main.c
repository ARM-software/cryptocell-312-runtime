		...
		uint8_t *pPubKeyFileName)
{
	...
	rc = CC_CommonCalcHBKFromFile(pPubKeyFileName, (uint8_t*)keyCertBody.nextPubKeyHash, HASH_SHA256_DIGEST_SIZE_IN_BYTES);
	if (rc != 0) {
		...
		uint8_t *pPubKeyFileName)
{
	...
	/* calculate SHA256 on public key file*/
	rc = CC_CommonCalcHBKFromFile(pPubKeyFileName, (uint8_t*)enablerCertBody.nextPubKeyHash, HASH_SHA256_DIGEST_SIZE_IN_BYTES);
	if (rc != 0) {
		UTIL_LOG_ERR("failed to CC_CommonCalcHBKFromFile \n");
		goto END;
	}
