...
class RsaCrypto:
    ...
    SB_CERT_RSA_KEY_SIZE_IN_BYTES = 384
    SB_CERT_RSA_KEY_SIZE_IN_WORDS = 96
    RSA_SALT_LEN = 32
    CC_PKA_WORD_SIZE_IN_BITS = 64  # The size of the PKA engine word.
    NP_SIZE_IN_BYTES = 20
    NP_SIZE_IN_WORDS = 5
    ...
    @staticmethod
    def generate_rsa_pem_key(pem_filename, password_filename=None):
        ...
        private_key = rsa.generate_private_key(public_exponent=65537,
                                               key_size=RsaCrypto.SB_CERT_RSA_KEY_SIZE_IN_BITS,
    ...
    @staticmethod
    def extract_public_rsa_pem_key(private_key, public_pem_filename):
        logger = logging.getLogger()
    ...
    @staticmethod
    def load_rsa_pem_key(pem_filename, is_key_private, keypair_passphrase_filename=None):
        """
        ...
        if is_key_private is True:
            if keypair_passphrase_filename is not None and len(keypair_passphrase_filename) > 0:
                try:
        ...
        try:
            if is_key_private is True:  # private key case
                ...
                if key.key_size != RsaCrypto.SB_CERT_RSA_KEY_SIZE_IN_BITS:
                    if logger is not None:
                        logger.warning("Key size does not match the expected %s bits" % RsaCrypto.SB_CERT_RSA_KEY_SIZE_IN_BITS)
                    raise RsaKeyLoadingError("Could not parse PEM file - key size is not expected")
    ...
    @staticmethod
    def calculate_np_from_n(n_as_modulus):
        logger = logging.getLogger()
        ...
        param_np = (2 ** RsaCrypto.SNP) // n_as_modulus
        return param_np

    @staticmethod
    def concatenate_n_and_np(n_as_modulus, param_np):
        logger = logging.getLogger()
        if not isinstance(n_as_modulus, int):
            if logger is not None:
                logger.warning("input parameter n_as_modulus expected to be integer")
        ...
        n_in_bytes = n_as_modulus.to_bytes(RsaCrypto.SB_CERT_RSA_KEY_SIZE_IN_BYTES, 'big')
        np_in_bytes = param_np.to_bytes(RsaCrypto.NP_SIZE_IN_BYTES, 'big')
        n_and_np_in_bytes = n_in_bytes + np_in_bytes
        return n_and_np_in_bytes

    @staticmethod
    def sign_pkcs_v21(prehashed_data_to_sign, rsa_key):
        signature = rsa_key.sign(prehashed_data_to_sign,
                                 padding.PSS(mgf=padding.MGF1(hashes.SHA256()),
                                             salt_length=RsaCrypto.RSA_SALT_LEN
                                             ),
                                 utils.Prehashed(hashes.SHA256()))
        return signature

    @staticmethod
    def verify_signature_pkcs_v21(signature, prehashed_message, public_key):
        """
    ...
    def load_rsa_public_key_from_n(n_as_modulus):
        if isinstance(n_as_modulus, int) is False:
            raise TypeError("Argument n_as_modulus must be an integer")
...
class Common:
    @staticmethod
    def get_n_and_np_from_public_key(public_key_filename):
        key = RsaCrypto.load_rsa_pem_key(public_key_filename, False)
        param_n = RsaCrypto.get_n_from_rsa_pem_public_key(key)
        param_np = RsaCrypto.calculate_np_from_n(param_n)
        n_and_np_bytes = RsaCrypto.concatenate_n_and_np(param_n, param_np)
        return n_and_np_bytes

    @staticmethod
    def get_hashed_n_and_np_from_public_key(public_key_filename):
        n_and_np_bytes = Common.get_n_and_np_from_public_key(public_key_filename)
    ...
    def get_n_and_np_from_keypair(keypair_filename, keypair_passphrase_file):
        key = RsaCrypto.load_rsa_pem_key(keypair_filename, True, keypair_passphrase_file)
    ...
    @staticmethod
    def rsa_sign(data_to_sign, keypair_filename, keypair_passphrase_file):
        key = RsaCrypto.load_rsa_pem_key(keypair_filename, True, keypair_passphrase_file)
        hashed_data_to_sign = HashCrypto.calculate_sha256_hash(data_to_sign)
        signature = RsaCrypto.sign_pkcs_v21(hashed_data_to_sign, key)
        RsaCrypto.verify_signature_pkcs_v21(signature, hashed_data_to_sign, key.public_key())
        return signature

    @staticmethod
    def rsa_sign_with_private_key(data_to_sign, private_key):
        hashed_data_to_sign = HashCrypto.calculate_sha256_hash(data_to_sign)
        signature = RsaCrypto.sign_pkcs_v21(hashed_data_to_sign, private_key)
        RsaCrypto.verify_signature_pkcs_v21(signature, hashed_data_to_sign, private_key.public_key())
        return signature

    @staticmethod
    def rsa_verify_with_pubkey_params(rsa_pubkey_n, signed_data, signature):
        modulus_in_bytes = rsa_pubkey_n[0:RsaCrypto.SB_CERT_RSA_KEY_SIZE_IN_BYTES]  # chop of np if input contains it
        ...
        RsaCrypto.verify_signature_pkcs_v21(signature, hashed_data_to_sign, public_key)

    @staticmethod
    def decrypt_data_with_rsa_keypair(keypair_filename, keypair_passphrase_file, ciphertext):
        private_key = RsaCrypto.load_rsa_pem_key(keypair_filename, True, keypair_passphrase_file)
    ...
    def encrypt_data_with_rsa_pubkey_params(rsa_pubkey_n_and_np, input_data):
        modulus_in_bytes = rsa_pubkey_n_and_np[0:RsaCrypto.SB_CERT_RSA_KEY_SIZE_IN_BYTES]
        modulus = int.from_bytes(modulus_in_bytes, "big")
        public_key = RsaCrypto.load_rsa_public_key_from_n(modulus)
    ...
    @staticmethod
    def encrypt_file_with_aes_ctr(input_filename, output_filename, key_int_array, aes_iv, hash_type):
        """
    ...
    @staticmethod
    def derive_aes_key(enc_password_filename):
        logger = logging.getLogger()
        # get passphrase
    ...
    @staticmethod
    def encrypt_asset_with_aes_cbc(input_filename, output_filename, enc_password_filename):
        logger = logging.getLogger()
        aes_key, aes_iv = Common.derive_aes_key(enc_password_filename)
    ...
    @staticmethod
    def decrypt_asset_with_aes_cbc(ciphertext, password_filename):
        aes_key, aes_iv = Common.derive_aes_key(password_filename)
